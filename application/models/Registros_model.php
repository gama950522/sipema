...
class Registros_model extends MY_Model
{
    ...
    public function add_data($data_value)
    {
        $data = $this->get_values($data_value);
        $this->db->trans_start();
        ...
        $reporte_sipema = array(
            ...
            '_id_presupuesto_desagregado'  => $ids['presupuesto_desagregado']['id'],
            '_id_programacion_actividades' => $ids['programacion_actividades']['id'],
            '_id_programacion_metas'       => $ids['programacion_metas']['id'],
            '_id_viaticos'                 => $ids['viaticos']['id'],
            '_id_cat_sub_programa'         => $data['hdn_id_subprograma'],
            '_id_usuario'                  => $this->session->userdata('id')
        );
        $this->db->query('call sp_new_reporte_sipema(?,?,?,?,?,?,?,?)', $reporte_sipema);
        // $this->db->insert('reporte_sipema', $object);
